...
const struct sys_mm_drv_region *sys_mm_drv_simple_query_memory_regions(void)
{
	...
	const static struct sys_mm_drv_region adsp_meteorlake_memory_regions[] = {
		{ .addr = &_end, .size = application_memory_size, .attr = MEM_REGION_SYS_HEAP},
		{ .addr = (uint32_t)&_end + application_memory_size, .size = region_unavalible_to_application_size, .attr = MEM_REGION_UNAVALIBLE_FOR_APPLICATION},
		NULL
	};
